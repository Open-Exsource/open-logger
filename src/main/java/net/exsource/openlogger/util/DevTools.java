package net.exsource.openlogger.util;
...
public class DevTools {
    ...
    public static void writeIni(@NotNull String path, @NotNull FileStorage... objects) {
        logger.info("Try loading/creating ini file " + path + "...");
        try {
            Ini ini = new Ini();
            StringBuilder builder = new StringBuilder();
            for (FileStorage storage : objects) {
                builder.append("[").append(storage.name()).append("]").append("\n");
                for (String key : storage.objects().keySet()) {
                    builder.append(key).append(" = ").append(storage.objects().get(key)).append("\n");
                }
            }

            String content = builder.toString();
            ini.load(content);
            ini.store(new FileWriter(path), "Last Update - " + getCurrentDate());
        } catch (IOException exception) {
            logger.error(exception);
        }
    }
    ...
    public static void readIni(@NotNull String path, @NotNull List<FileStorage> stored) {
        logger.debug("Try loading ini file " + path + "...");
        try {
            Ini ini = new Ini();
            if (path.startsWith("@")) {
                ini.load(path.substring(1));
            } else {
            ...
            for(String entry : sections) {
                ...
                Map<String, Object> section = ini.getSection(entry);
                for(String key : section.keySet()) {
                    tmp.put(key, section.get(key));
                }
                FileStorage storage = new FileStorage(entry, tmp);
                stored.add(storage);
            }
        } catch (IOException exception) {
            logger.error(exception);
    ...
    /**
     * This method simple get the current date in this format: dd.MM.yyyy HH:mm:ss
